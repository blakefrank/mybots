...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        print("Original value:", self.Get_Value())
        # print(neurons)
        # print(synapses)
        self.Set_Value(0)
        for key in synapses.keys():
            pre_synapse_name = key[0]
            post_synapse_name = key[1]
            if post_synapse_name == self.Get_Name():
                weight = synapses[key].Get_Weight()
                presynaptic_value = neurons[pre_synapse_name].Get_Value()
